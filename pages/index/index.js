...
var getauthorize= null;   //用户是否授权定时器
var once =true; //int调一次
...
Page({
    data: {
        ...
        ruleText: [
            ...
            '答错题目可以转发到不同微信群换取立即复活并继续答题的机会，每次答题最多有3次复活机 会，大于3次可以购买复活卡继续答题，每次挑战最多可购买3次。',
            '领取活动奖品需关注我们的微信公众号并联系在线客服。',
            '如有任何问题请联系我们官方公众号“智汇酷玩”。'
        ], // 规则
        gametext: [
            '一口气答对12题即可领取娃娃奖品',
        ...
        frequency: 0 ,//次数
        show_share:false,
        showGetinfo:false,    //用户是否授权,true为未授权
        // 夺宝
        // 总次数
        total: 23414,
        // 标题
  ...
  // 分页
    ...
    init(){
        ...
        if (getauthorize) clearInterval(getauthorize)
            this.toload(percent);
            mask = true;
    ...
    initial: function (res) {
        if (res.data.code == 20000) {
            ...
            // 无挑战机会弹框按钮开关
            app.globalData.flag = !data.flag ? 1 : data.flag;
            // 复活开关
            app.globalData.flag2 = !data.flag2 ? 1 : data.flag2;
            //群内智力开关
            app.globalData.flag3 = !data.flag3 ? 1 : data.flag3;
            //个人中心炫耀战绩开关
            app.globalData.flag4 = !data.flag4 ? 1 : data.flag4;
            //挑战完成"获得更多挑战机会"开关
            app.globalData.flag5 = !data.flag5 ? 1 : data.flag5;
            //"分享到群立即复活"按钮开关
            app.globalData.flag6 = !data.flag6 ? 1 : data.flag6;
    ...
    toadv: function (e) {
        ...
        var item = this.data.adv[index];
        var url = item.link_value
        if (item.type == 0){
            return false
        }
        if (item.type == 1) {
            wx.setStorageSync('advertiseUrl', url)
            wx.navigateTo({
